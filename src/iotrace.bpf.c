...
// trace target device
static inline int common_filter(
        pid_t tid, pid_t tgid, dev_t dev, ino_t inode, ino_t dir_inode) {
    if (target_tgid != 0) {
...
static inline void set_common_info(struct event* task_info,
                                   pid_t tgid,
                                   pid_t tid,
...
static inline void set_fs_info(struct event* task_info,
                               ino_t inode,
                               ino_t dir_inode,
                               dev_t dev,
...
static inline int read_and_store_abs_path(struct path* p,
                                          ino_t* inode,
    ...
    bpf_probe_read_kernel_str(
            &tp->disk_name[0], 40, BPF_CORE_READ(disk, disk_name));
...
int BPF_PROG(enter_vfs_read) {
    ...
    if (bpf_map_lookup_elem(&ino_path_map, &i_ino) == NULL) {
        if (!read_and_store_abs_path(
                    &p, &i_ino, BPF_CORE_READ(inode, i_sb, s_bdev))) {
            goto go_on;
        }
    }
go_on:;
...
int BPF_PROG(enter_filemap_get_pages,
             struct kiocb* iocb,
...
int BPF_PROG(exit_filemap_get_pages,
             struct kiocb* iocb,
             struct iov_iter* iter,
             struct pagevec* pvec) {
    struct file* file = BPF_CORE_READ(iocb, ki_filp);
...
int BPF_PROG(trace_enter_filemap_write_and_wait_range,
    ...
    set_common_info(task_info,
                    tgid,
                    tid,
                    filemap_write_and_wait_range_enter,
                    vfs_layer);
...
int BPF_PROG(trace_exit_filemap_write_and_wait_range,
    ...
    set_common_info(
            task_info, tgid, tid, filemap_write_and_wait_range_exit, vfs_layer);
    set_fs_info(task_info, i_ino, 0, 0, start_byte, end_byte + 1 - start_byte);
    bpf_ringbuf_submit(task_info, 0);
    return 0;
}
...
int BPF_PROG(trace_enter_filemap_range_needs_writeback,
    ...
    set_common_info(task_info,
                    tgid,
                    tid,
                    filemap_range_needs_writeback_enter,
                    vfs_layer);
    set_fs_info(task_info, i_ino, 0, 0, start_byte, end_byte + 1 - start_byte);
    bpf_ringbuf_submit(task_info, 0);
    return 0;
...
int BPF_PROG(trace_enter_iomap_dio_rw,
             struct kiocb* iocb,
             struct iov_iter* iter) {
    struct file* file = BPF_CORE_READ(iocb, ki_filp);
    u64 id = bpf_get_current_pid_tgid();
...
int BPF_PROG(trace_exit_iomap_dio_rw,
             struct kiocb* iocb,
             struct iov_iter* iter) {
    struct file* file = BPF_CORE_READ(iocb, ki_filp);
    u64 id = bpf_get_current_pid_tgid();
...
static inline void set_rq_comm_info(struct event* task_info,
                                    struct request* rq,
                                    dev_t dev) {
...
static inline void set_common_bio_info(struct event* task_info,
                                       struct bio* bio,
                                       dev_t dev) {
    ...
    for (int i = 0; i < MAX_BIO_PER_RQ; i++) {
        ...
        if (bi_iter.bi_size <= nr_bytes) {
            if (bio_flag & BIO_TRACE_MASK) {
                ...
                task_info->bio_rq_association_info.bio =
...
int BPF_KPROBE(trace_rq_qos_track,
               struct rq_qos* q,
               struct request* rq,
               struct bio* bio) {
...
int BPF_KPROBE(trace_rq_qos_merge,
               struct rq_qos* q,
               struct request* rq,
               struct bio* bio) {
    // dev_t dev = BPF_CORE_READ(bio, bi_bdev, bd_dev);
