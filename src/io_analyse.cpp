...
void IOAnalyser::AddTrace(struct event *e) {
  ...
  if (e->event_type == vfs_read_enter || e->event_type == vfs_write_enter) {
    ...
    request->AddEvent(std::move(event));
    AddRequest(std::move(request));
  } else if (e->event_type == submit_bio) {
    // new bio ,may belong to more than one request
    // if(bio_map.find(e->bio_info.bio) != bio_map.end()){
    //   printf("bio already exist pid %d, tid %d, comm
    //   %s\n",e->pid,e->tid,e->comm); return;
    // }
    if (bio_map.find(e->bio_info.bio) != bio_map.end()) {
    ...
    // find the request
    for (auto &request : pending_requests) { // NOTE: make sure pending_requests
  ...
  } else if (e->event_type == bio_endio) {

    if (bio_map.find(e->bio_info.bio) == bio_map.end()) {
      printf("bio not found pid %d, tid %d, comm %s\n", e->pid, e->tid,
             e->comm);
      return;
    }
    ...
    for (int i = 0; i < pending_requests.size(); i++) {
      auto &request = pending_requests[i];
      IORequest *io_request = dynamic_cast<IORequest *>(request.get());
      bool isDone = true;
      isDone = io_request->isRelative(e->vfs_layer_info.inode,
