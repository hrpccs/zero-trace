        ...
        if (auto syncevent = dynamic_cast<SyncEvent*>(e)) {
            ...
            if (enterIter != event_pair_map.end()) {
                event_pair_occur_map[enterIter->second] =
                        syncevent->timestamp; // store enter's timestamp to exit
                                              // event
            } else {
                // printf("exit event1: %s\n",
                // kernel_hook_type_str[syncevent->event_type]);
                auto exitIter =
                        event_pair_occur_map.find(syncevent->event_type);
                if (exitIter != event_pair_occur_map.end()) {
                    updateStatistic(syncevent->event_type,
                                    syncevent->timestamp,
                                    exitIter->second,
                                    iorequest->type);
...
void IOEndHandler::HandleDoneRequest(std::shared_ptr<Request> request) {
    ...
    int tapnum = 0;
    //   if (inode_abs_path_map.find(iorequest->inode) ==
    //   inode_abs_path_map.end()) {
    //     fprintf(outputFile,
    //             "task %s  request %lld total time %lf \ttarget file inode
    //             %lld " "path %s\n", iorequest->comm.c_str(), iorequest->id,
    //             duration_ms, iorequest->inode, "unknown");
    //   } else {
    //     fprintf(outputFile,
    //             "task %s  request %lld total time %lf \ttarget file inode
    //             %lld " "path %s\n", iorequest->comm.c_str(), iorequest->id,
    //             duration_ms, iorequest->inode,
    //             inode_abs_path_map[iorequest->inode].c_str());
    //   }
    fprintf(outputFile,
            "task %s  request %lld total time %lf \ttarget file inode %lld \n",
            iorequest->comm.c_str(),
            iorequest->id,
            duration_ms,
            iorequest->inode);
    ...
    for (int i = 0; i < iorequest->events.size(); i++) {
        ...
        if (auto syncevent = dynamic_cast<SyncEvent*>(e)) {
            // printf("sync event: %s\n",
            // kernel_hook_type_str[syncevent->event_type]); first print the
                    ...
                    if (exitIter != event_pair_occur_map.end()) {
                        ...
                        if (enterIter != event_pair_map.end()) {
                            fprintf(outputFile,
                                    ...
                                    timestamp2ms(syncevent->timestamp - start),
                                    tapstr.c_str(),
                                    kernel_hook_type_str[syncevent
                                                                 ->event_type]);
                        } else {
                            fprintf(outputFile,
                                    "%10f ms%s} %s\n",
...
void IOAnalyser::processVfsExit(struct event*& e,
    ...
    for (int i = pending_requests.size() - 1; i >= 0; i--) {
        ...
        if (io_request->isRelative(e->pid,
                                   e->tid,
                                   e->vfs_layer_info.inode,
...
void IOAnalyser::AddTrace(void* data, size_t data_size) {
    if (data_size == sizeof(struct bvec_array_info)) {
        ...
        if (e->info_type == bio_rq_association_info) {
            if (e->event_type == rq_qos_track ||
                e->event_type == rq_qos_merge) {
                ...
                addEventToBio(e->bio_rq_association_info.bio, event);
                deleteBioRqAssociation(
                        e->bio_rq_association_info.bio,
                        e->bio_rq_association_info.rq,
                        e->bio_rq_association_info.request_queue);
            } else {
            ...
            auto event = std::make_shared<SyncEvent>(
                    e->event_type, e->timestamp, std::string(e->comm));
            if (e->event_type == block_rq_insert ||
                e->event_type == block_rq_issue ||
            ...
            if (e->event_type == block_bio_queue) {
                ...
                processBioSplit(e->bio_info.bio,
                                e->bio_info.parent_bio,
                                e->bio_info.bvec_idx_start,
                                e->bio_info.bvec_idx_end);
                addEventToBio(e->bio_info.bio, event);
            } else {
                printf("IOAnalyser::AddTrace: unknown event type %s\n",
                       kernel_hook_type_str[e->event_type]);
