...
class IOEndHandler : public DoneRequestHandler {
  public:
        ...
        std::thread statistic_thread([this]() {
            while (true) {
                std::this_thread::sleep_for(std::chrono::seconds(
                        this->config.timer_trigger_duration));
    ...
    void readAbsPath(unsigned long long inode,
                     std::string& path,
    ...
    void getAvgStatisticAndReset() {
        ...
        avg_write_request_time = (double)write_request_time_sum /
                                 write_request_count / 1000000.0;
        avg_get_page_time =
                (double)get_page_time_sum / get_page_count / 1000000.0;
        avg_read_offcpu_time =
  ...
  public:
    ...
    void AddRequestObject(unsigned long long rq,
        ...
        if (request_queue_map.find(request_queue) == request_queue_map.end()) {
            request_queue_map[request_queue] =
                    std::make_shared<RequestQueueObject>();
        }
        ...
        request_queue_object->request_objects.push_back(
                std::move(request_object));
    }

    // must have bio queue or split first
    bool addBioRqAssociation(unsigned long long bio,
                             unsigned long long rq, // for add bio to request,
                                                    // merge bio with request
                             unsigned long long request_queue) {
    ...
    bool processBioQueue2(unsigned long long bio,
        ...
        for (int i = 0; i < bvec_info->bvec_cnt; i++) {
            ...
            auto offset = (bvec_info->bvecs[i].index << 12) +
                          bvec_info->bvecs[i].bv_offset;
            auto len = bvec_info->bvecs[i].bv_len;
            bio_object->addAssociation(inode, offset, len);
            // printf("bio %lld bvec %d inode %lld offset %lld len %lld bvec cnt
            // %d\n", bio,i ,inode,
    ...
    bool processBioSplit(unsigned long long bio,
        ...
        for (int i = bvec_idx_start; i <= bvec_idx_end; i++) {
            ...
            for (auto& request_object: association.relative_requests) {
                // assert(request_object.expired() == false && "request not done
                // yet");
    ...
    bool addEventToBio(unsigned long long bio,
