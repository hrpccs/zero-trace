...
void VSockEngine::registerHelpers() { REGISTER_MANY_HELPERS }

int VSockEngine::sendstr(char *str, enum Type type, int len) {
    // printf("sendstr len:%d type %d\n",len,type);
...
int VSockEngine::sendMesg(enum Type type, void *obj) {
  char *buf;
  int actural_len = send_helper[type](
      obj, buf); // 生成字符串,因为类里面可能有指针,导致直接强转会出问题
    // printf("sendMesg len:%d\n",actural_len);
...
ClientEngine::~ClientEngine() { close(mysock); }
