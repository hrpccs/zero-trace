...
初赛中，我们虽然完成了全链路追踪，但是因为过滤机制不够高效，导致性能问题突出。
...
部分实现如下，以对 read 系统调用的追踪为例。

在系统调用 read 的入口处，获取过滤所需信息，然后根据过滤条件判断该系统调用是否会被追踪。若会被追踪，则在 eBPF Hash Maps 中记录下该线程号，为后续同步事件的过滤提供先验信息。则退出 read 系统调用时，通过 tid 查询 Map 中是否有值，若有，则说明该事件是某个正在被追踪的；若无，则丢弃。
...
设计有上述的过滤机制后，可以实现各种条件下的过滤器。

思路：在系统调用的入口处，获取与过滤条件相关的信息，然后进行过滤，若不被过滤，则把当前线程加入到活动上下文的记录中。
...
在使用 eBPF 实现了过滤器功能之后，用户在运行 zero-tracer 时，就可以根据自己希望追踪的 IO 请求的特征来设置过滤条件，从而实现低开销。
...
下面介绍我们在各个子模块选取的挂载点，以及它们之间的关联信息
