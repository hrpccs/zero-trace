...
eBPF-tracing工具是基于eBPF技术实现的内核跟踪工具。它能够有效地获取系统应用程序与内核之间的额外诊断信息，从而帮助工程师分析程序在内核态的处理流程，是云原生可观测的一类重要工具。如今已经有不少较为成熟的ebpf-tracing工具，如 bcc、bpftrace 等。

但是其中 bcc 工具的是着眼于内核中某一个子模块的 tracing，没有工具能够很好地进行内核请求的跨越多个子系统的路径追踪。而 bpftrace 也只提供比较简单地自定义逻辑，没办法很好地把不同内核子模块的 trace 关联起来。
...
为了解决这个问题，本项目实现了一个名为 zero-tracer 的 eBPF tracing 工具，它能够追踪 read/write 等 IO 系统调用在内核中各个子模块中的处理流程，并且在 12k read iops 和 250 write iops 环境下能保持小于 5% 的系统 CPU 开销。zero 表示低开销。
...
1. 实现一个针对内核数据处理流程的通用跟踪框架，基于该框架可方便实现针对某个/某些数据结构(如bio, request, skb , page 等)在内核中的处理流程跟踪，并生成延时分析报告。（**已完成对 bio，request，page 等 IO 相关数据结构的追踪**）
2. 实现存储IO请求全链路跟踪。
...
4. 统计运行开销(内存，cpu)，为保证工具可用于常态部署，控制运行时单核及全系统cpu开销<5%，并尽量优化。（在 **read iops = 12k，write iops = 250  的同步 IO 测试环境下，每核开销 < 5%，具体性能开销分析见性能分析报告**）
...
项目要求实现的功能要求我们的工具，能够把存储 IO 请求的全链路处理流程追踪出来，并且是请求级别的。其中包括在系统调用和文件系统层的同步事件、还包括涉及到的异步数据结构 bio、request、page 的各自的处理流程。而 bio 和 request 的处理流程有可能去往不同的设备驱动，如 nvme，scsi，virtio 设备驱动，所以还要能够判断和区分这个存储 IO 请求具体是加入了什么驱动，在驱动内部是如何处理的。

同时为了降低性能开销，我们需要提供各个维度的过滤器，以及利用 libbpf 的各种特性，把我们基于 eBPF 开发的 tracing 工具的开销控制在每核 5% 以内。

对于拓展功能中的 virtio 从 guest 到 host 的全链路追踪，则是因为 virtio 的半虚拟化设计，使得 guest OS 在对 virtio 设备读写时的具体数据可以被 qemu 在 Host OS 的用户态空间截获，从而建立起 virtio 在 guest OS 的块设备存储请求在 qemu 中转换成 Host OS 的块设备请求的关联。

## A、项目整体架构设计

我们的项目希望能抓取到内核的存储 IO 在内核中各个子模块的执行路径，我们把一个请求抽象成若干个有序事件的列表，要完成对请求的追踪，就需要把从内核中获取到的无序事件集合关联成合理的有序的事件列表。

为了阐述我们是如何完成这个项目的，我们将整个项目分为**数据采集、数据过滤与关联、对完整请求的处理三个模块**。下面分别对这三部分需要做的事情进行介绍：

- **数据采集**为我们提供事件，每个事件包含时间戳与事件类型，以及提供关联不同 trace 事件时需要的上下文信息。在 eBPF 语境下，数据源就是 eBPF 挂载点，每次挂载点触发时，都会触发一段 eBPF 程序，从内核获取数据。所以**我们需要在 eBPF 程序中采集我们想要的数据。**
- **数据过滤与关联机制**，是指数据采集阶段为我们提供了诸多事件与事件附加的上下文信息，由于事件传输和处理都需要开销，我们希望尽早判断某些事件是无用的，**即过滤机制**。过滤剩下的事件则潜在地可以从无序的事件集合按照事件的上下文信息间的关联，把若干个事件关联成一个完整的请求的执行路径，是**为关联机制**。所以这部分**我们需要从采集的数据中维护上下文信息，过滤数据以及把已有事件关联起来。**
- **对完整请求的处理**，是指在不同事件完成关联后，得到了有序的可以反映执行流程的一个完整的请求，对这个请求应该如何处理。可以对该请求进行统计、时延分析、还可以决定它的下一步去向。而这部分**我们需要为不同的去向进行代码适配，从而可以把我们的系统输出接入到其它系统，从而达成系统间的协作。**
- 其中**数据采集机制和数据过滤与关联机制之间存在相互影响**：由于数据采集本身存在开销，所以希望能够以尽量少地数据就能为数据过滤与关联提供有效信息，这就要求数据源本身既能反映请求执行流，又要能提供有效的信息给上层机制。而数据过滤与关联机制中的过滤，往往能够减少不必要的数据采集，即在某个数据源采集到的数据本身，结合已有的数据，即可以判断接下来的数据是否需要采集。

在搞清楚项目的构成之后，**各部分需要的实际工作可以总结如下**：

- 在 Linux 存储 IO 请求会涉及的子模块中寻找 eBPF 挂载点，并且为每个 eBPF 挂载点编写相应的事件处理程序。
...
先讨论过滤器的设计，然后讨论如何在过滤器的框架下，进行存储 IO 的事件关联。
...
目前 blktrace 工具是分析 linux 系统 IO 请求在 block layer 与 driver layer 的活动情况的强大工具，但是它只支持对整个块设备的 IO 请求的数据采集与关联。而对于我们还要依据对 bio、request 数据的追踪完成对更细维度的追踪比如线程、进程等，所以 blktrace 提供的功能过于粗粒度，所以没办法直接用 blktrace 完成我们的任务。但是了解 blktrace 内部实现后，会发现其中用到了的 tracepoint 可以提供很好参考。
...
在初赛提交的代码中，我们尝试模仿 blktrace 的思路：利用 eBPF 从 block_bio_queue 等 tracepoint 获取数据，在用户态维护 request 和 bio 的 1 对 多关系，从而维护了 bio 和 request 数据结构在 block layer 与 driver layer 子模块的处理流程。详情可查看我们的初赛文档。
...
**初赛中，我们过于关注事件在不同子模块之间的关联反而忽略了过滤机制的重要性。**在初赛中，我们根据线程号、进程号、磁盘设备号进行过滤时，仅仅是在挂载点中尝试获取与这些过滤条件直接有关的数据，比如线程的 tid、pid，IO请求的目标文件的磁盘设备号，然后进行判断，若条件符合则保留，否则就过滤丢弃。对于 fs 层的单独一个请求来说，这个简单的过滤已经足够了，比如下面的 vfs_read 和 filemap_get_pages 两个 fs 层挂载点，单独来看，都时根据用户态设置的过滤条件进行过滤，实际上没有发掘上下文中潜在的过滤信息——**对于同步 IO 来说，如果 vfs_read 已经获取过了线程/进程号，以及读写目标 inode，dir_inode，dev_id，cgroup 这些信息，并且经过过滤条件判断是保留的，那么后续属于该请求同步事件，实际上都不需要重复获取 inode/dir_inode/dev_id/cgroup_id 等与上下文无直接关联的信息。**

如下图，两个挂载点都用比较复杂的逻辑获取很多数据，但是实际上很多是非必要的。
...
现在对我们实现的过滤机制进行描述，以及阐述如此设计的原因：

**过滤机制：一、分为对同步事件的过滤 二、对 page，bio，request 等异步数据结构的过滤**

一、同步事件的过滤指：在某上下文发起同步 IO 系统调用时，判断该上下文是否属于我们过滤器的过滤目标，若是，直接丢弃；若不是，则记录当前的上下文为活动上下文。把活动上下文的 tid 记录在 **eBPF Hash Map** 中，在该请求结束时清楚该记录。则该如果某个线程的 tid 位于该 Map 中，则说明该线程正处于一个同步的 IO 请求中，而不需要再获取 inode 之类的信息再与过滤条件判断。

具体实现如下，以对 read 系统调用的追踪为例，在系统调用的入口处，获取过滤所需信息，然后根据过滤条件判断该系统调用是否会被追踪。若会被追踪，则在 eBPF Hash Maps 中记录下该线程号，为后续同步事件的过滤提供先验信息。则退出 read 系统调用时，通过 tid 查询 Map 中是否有值，若有，则说明该事件是某个正在被追踪的；若无，则丢弃。
...
int BPF_KPROBE_SYSCALL(trace_read, int fd, void *buf, size_t count) {
  ...
  // 判断该请求不会被过滤后，把当前任务的线程号 tid 存入 tid_syscall_enter_map 中，说明该线程正在执行同步IO操作。
...
int BPF_KRETPROBE(trace_read_ret, int ret) {
  ...
  if (get_and_filter_pid(&tgid, &tid)) {
   ...
   // 判断当前线程是否处于某个同步 IO 中，若是，则删除其中记录，若不是则丢弃事件。
...
二、对 page，bio，request 等异步数据结构的过滤：是根据已经发生的 fs 层同步事件判断系统中哪些 page，bio，request 是活动的，并记录在 eBPF Hash Map 中，并且在活动数据结构在内核上下文的处理流程中，根据触发的事件类型与事件语义维护活跃数据结构记录的增删改查。 

> 对于发起同步 IO的上下文来说（包括，read/write 、pread/write、fsync/fdatasync 等），导致请求时延抖动的情况往往是本请求发现 pagecache miss 以及 dirty 而主动从块设备读入以及等待脏页写回块设备，这种情况下导致请求时延波动的 bio 请求就是由本上下文发起的。
>
> 而对于 epoll，io_uring，AIO 等使用了比较复杂的机制，其在现代软件提高 IO 性能中有着重要意义，但是因为机制与普通的读写差异较大，要求我们对这些机制由更深入的了解才能很好的设计追踪与过滤机制，本项目目前不涉及。
>
> block_bio_queue  tracepoint 实际上是由与发起同步 IO 的上下文触发的。

具体的实现上，我们发现 block_bio_queue 是同步请求发出 bio 的开始点，这是发生在同步 IO 的上下文中的，所以该事件发生时，判断所在上下文的线程号，然后把涉及到的 bio 的 id 存入到 eBPF Map 中，后续与 bio 相关的事件可根据该 eBPF Map 中是否存有 bio 的 id 来判断是否为活跃 bio。并且根据活跃的 bio 来确定活跃的 request，系统中存在挂载点可以判断何时 bio 加入到一个 request 里，比如内核函数 \_\_rq_qos_track(struct bio\*, struct request\*) 。所以选出这些代表着 bio 和 request 各自的生命周期与数据结构间的关联的挂载点，是我们一个重要的工作方面（详情见挂载点文档 doc/hook_point.md）。

维护 bio 生命周期，bio 与 request 相关联，bio 结束的挂载点中，都维护了活动 bio 和 request 的记录，从而帮助过滤无用的 bio 和 request。
...
在设计有上述的过滤机制后，实现各种条件下的过滤器变的简单。

只需要在系统调用的入口处，获取与过滤条件相关的信息，然后进行过滤，若不被过滤，则把当前线程加入到活动上下文的记录中。
...
以下为实现上述功能的 eBPF 程序，具体功能的展示和验证，可以看 doc/functional-test.md 文档。
...
在使用 eBPF 实现了过滤器功能之后，我们希望用户在运行我们的 zero-tracer 时，就可以根据自己希望追踪的 IO 请求的特征来设置过滤条件，从而实现较低的开销。
...
但是我们在内核中的过滤依据除了 pid 和 tid 还有就是 cgroup_id, 文件或者目录的 inode number，块设备的设备号。

所以我们对用户的输入进行转换（位于 src/utils.cpp)，分别把**用户指定的路径转换成对于的 cgroup_id, inode 号，设备号。**
...
然后借助 libbpf 的全局变量特性，在加载 eBPF 程序之间把这些过滤条件设置好，让内核态的 eBPF 程序可以感知到用户设置过滤条件。

### 2.（基础功能）挂载点选取与事件关联 

实际上，挂载点选取、数据关联、过滤机制的实现是一个整体，是相互影响的。
...
上述工作中，我们重新设计的过滤器，而过滤器本身获取过滤所需信息需要特定的挂载点才能提供，过滤器本身把活动上下文，bio，request，page 等记录下来，其实也是用于事件关联的。

此外，挂载点的选取决定了我们的 zero-tracer 能够覆盖的 IO 请求的执行路径，比如在请求处理过程中存在两个关键分支的函数，如果只挂载其中一个就不能很好地反映请求的执行路径。但是挂载点选取地越多，事件触发就会越多，eBPF 程序带来的开销也越大。
...
下面介绍我们在存储 IO 请求经过的各个子模块设置的挂载点以及理由：
...
具体涉及的挂载点在 `src/include/hook_point.h` 中定义类型，前缀为 `syscall__*`，在 `src/iotrace.bpf.c` 中实现相应的 eBPF 程序. 具体语义在 `doc/io_hookpoint.md` 中有详细描写。
...
涉及到挂载点的语义可在 `doc/io_hookpoint.md` 中查看。
...
对于同步 IO 来说，syscall 和 fs 层的时间可以根据活动上下文关联、对于 pagecache 层，通过 page id 即 page 数据结构的地址来关联、对于 block ，各个 driver 层，则通过 bio 和 request 的 id 即地址来关联。
...
用户态程序在处理每一个事件时，根据事件类型来选择用什么信息进行关联。可以从结构体看到主要通过 tid，tgid，rq_id，bio_id 来进行事件关联。
...
我们在设计的时候，考虑到可能会把我们抓取到的 trace 信息进行不同的通知。比如上传到 grafana 进行更美观的前端展示，还有打印日志到某一文件，亦或者把整个请求发送到 host OS，让 host 端 zero-tracer 进一步处理。
...
还有一个是结合 opentelemety 进行把消息发送到 grafana ，实现远程 web 展示。
...
### 5. (拓展功能)实现 virtio 的 guest 到 host 全链路追踪
...
简单的功能展示如下（具体可以到 [功能测试文档](./doc/functional-test.md)
